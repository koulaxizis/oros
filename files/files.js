...
(function () {
  ...
  function renderEntries(entries) {
    ...
    entries.forEach(function (e, idx) {
      ...
      var sz = (e.size || e.bytes || 0);
      sizeSpan.textContent = e.dir ? "-" : fmtKB(sz);
      li.appendChild(sizeSpan);

      // Modified column
      var dateSpan = document.createElement("span");
      dateSpan.className = "col-date";
      var m = (e.mtime || e.modified || 0);
      dateSpan.textContent = m ? new Date(m).toLocaleDateString(LANG === "el" ? "el-GR" : "en-US", { day: "2-digit", month: "2-digit", year: "2-digit" }) : "-";
  ...
  function findMatchingRecursively(dir, q, token) {
    return fsList(dir).then(function (entries) {
      ...
      entries.forEach(function (e) {
        ...
        if (e.dir) {
          matches = matches.concat(findMatchingRecursively(path, q, token));
        }
      });
      return matches.slice(0, SEARCH_CAP);
    }).catch(function () {
      return (token !== renderToken) ? [] : [];        // unreadable branch → skip
    });
  }
